...
> 如需在 **VSCode**，或基于 VSCode 开发的 **Cursor** 等编辑器中使用，请前往 **https://github.com/GuangLang/readbook**。
...
1. **右键**状态栏上的插件项 → 弹出操作菜单：
   ...
   > **提示**：因为底部状态栏的右键已被 IDE 自身占用（用于编辑器右键菜单），所以**单次右键有时不会弹出本插件的菜单**；请**长按右键**（按住鼠标右键约 0.3 秒）即可看到如上菜单。
...
| **自动播放速度（毫秒/字，越小越快）** | 自动翻页的节奏。每段等待时间 = 速度 × 该段字数（最少 200ms）。默认 **100**（约 5 秒/段），数值越小翻得越快。 |
